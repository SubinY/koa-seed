 ...
 */
  ...
  function clearEmpty(o) {
    for (var key in o) {
      ...
      if (!o[key]||o[key] === null || o[key] === '' || o[key] === undefined || o[key] === '[]') {
        delete o[key]
      }
...
const del = (model, where, ctx) => {
    return model.findOneAndDelete(where)
        .then(res => {
            if (res) {
                ctx.body = {
                    code: 200,
...
const updateMany = (model, where, params, ctx) => {
    return model.updateMany(where, params)
        .then(res => {
            if (res) {
                ctx.body = {
                    code: 200,
...
const find = (model, where, ctx) => {
    ...
    console.log('原始列表查询条件where结果===>',JSON.stringify(where))

    clearEmpty(where)
    console.log('列表查询条件where结果===>',JSON.stringify(where))
    return model.find(where).skip((pageIndex - 1) * pageSize || 0).limit(pageSize || 0)
        .then(res => {
            if (res) {
                ctx.body = {
                    code: 200,
