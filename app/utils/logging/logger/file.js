...
export class FileTransport extends Transport {
  ...
  renameLogFile(filename) {
    ...
    const refilename = path.join(dir, `${today.format('YYYY-MM-DD')}-${mill}.log`);
  ...
  _createStream() {
    ...
    const onError = err => {
      console.error('%s ERROR %s [logger] [%s] %s', utility.logDate(','), process.pid, filename, err.stack);
      this.reload();
      console.warn('%s WARN %s [logger] [%s] reloaded', utility.logDate(','), process.pid, filename);
    };
